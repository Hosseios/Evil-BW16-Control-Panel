...
class EvilBW16GUI:
    ...
    def setup_command_frame(self):
        ...
        # Create buttons with equal width and grid layout
        button_width = 150  # Fixed width for all buttons
        self.start_button = ctk.CTkButton(button_frame, text="Start", width=button_width, command=lambda: self.send_command("start"))
        self.start_button.grid(row=0, column=0, padx=2, pady=2)

        self.stop_button = ctk.CTkButton(button_frame, text="Stop", width=button_width, command=lambda: self.send_command("stop"))
        self.stop_button.grid(row=0, column=1, padx=2, pady=2)

        self.scan_button = ctk.CTkButton(button_frame, text="Scan", width=button_width, command=lambda: self.send_command("scan"))
        self.scan_button.grid(row=0, column=2, padx=2, pady=2)

        self.results_button = ctk.CTkButton(button_frame, text="Results", width=button_width, command=lambda: self.send_command("results"))
        self.results_button.grid(row=0, column=3, padx=2, pady=2)

        self.info_button = ctk.CTkButton(button_frame, text="Info", width=button_width, command=lambda: self.send_command("info"))
        self.info_button.grid(row=0, column=4, padx=2, pady=2)

        self.help_button = ctk.CTkButton(button_frame, text="Help", width=button_width, command=lambda: self.send_command("help"))
        self.help_button.grid(row=0, column=5, padx=2, pady=2)
        ...
        # First row of parameters with fixed widths
        entry_width = 100
        label_width = 120

        ctk.CTkLabel(params_frame, text="Cycle Delay (ms):", width=label_width).grid(row=0, column=0, padx=2, pady=2)
        self.cycle_delay_entry = ctk.CTkEntry(params_frame, width=entry_width)
        self.cycle_delay_entry.grid(row=0, column=1, padx=2, pady=2)
        self.cycle_delay_entry.insert(0, "2000")

        ctk.CTkLabel(params_frame, text="Scan Time (ms):", width=label_width).grid(row=0, column=2, padx=2, pady=2)
        self.scan_time_entry = ctk.CTkEntry(params_frame, width=entry_width)
        self.scan_time_entry.grid(row=0, column=3, padx=2, pady=2)
        self.scan_time_entry.insert(0, "5000")

        ctk.CTkLabel(params_frame, text="Num Frames:", width=label_width).grid(row=0, column=4, padx=2, pady=2)
        self.num_frames_entry = ctk.CTkEntry(params_frame, width=entry_width)
        self.num_frames_entry.grid(row=0, column=5, padx=2, pady=2)
        self.num_frames_entry.insert(0, "3")

        self.apply_params_button = ctk.CTkButton(params_frame, text="Apply", width=button_width)
        self.apply_params_button.grid(row=0, column=6, padx=2, pady=2)

        # Second row of parameters
        ctk.CTkLabel(params_frame, text="Start Channel:", width=label_width).grid(row=1, column=0, padx=2, pady=2)
        self.start_channel_entry = ctk.CTkEntry(params_frame, width=entry_width)
        self.start_channel_entry.grid(row=1, column=1, padx=2, pady=2)
        self.start_channel_entry.insert(0, "1")

        ctk.CTkLabel(params_frame, text="Scan Between Cycles:", width=label_width).grid(row=1, column=2, padx=2, pady=2)
        self.scan_cycles_var = ctk.StringVar(value="off")
        self.scan_cycles_combo = ctk.CTkComboBox(params_frame, values=["on", "off"], variable=self.scan_cycles_var, width=entry_width)
        self.scan_cycles_combo.grid(row=1, column=3, padx=2, pady=2)

        ctk.CTkLabel(params_frame, text="LEDs:", width=label_width).grid(row=1, column=4, padx=2, pady=2)
        self.led_var = ctk.StringVar(value="on")
        self.led_combo = ctk.CTkComboBox(params_frame, values=["on", "off"], variable=self.led_var, width=entry_width)
        self.led_combo.grid(row=1, column=5, padx=2, pady=2)

        # Configure grid columns to have equal weight
        for i in range(7):
            params_frame.grid_columnconfigure(i, weight=1)

        # Target frame with grid layout
        target_frame = ctk.CTkFrame(frame, corner_radius=0)
        target_frame.pack(fill="x", padx=5, pady=2)

        ctk.CTkLabel(target_frame, text="Target Indices (comma-separated):", width=label_width*1.5).grid(row=0, column=0, padx=2, pady=2)
        self.target_entry = ctk.CTkEntry(target_frame, width=entry_width*3)
        self.target_entry.grid(row=0, column=1, padx=2, pady=2)

        self.set_target_button = ctk.CTkButton(target_frame, text="Set Target", width=button_width)
        self.set_target_button.grid(row=0, column=2, padx=2, pady=2)

        # Custom command frame
        custom_frame = ctk.CTkFrame(frame, corner_radius=0)
        custom_frame.pack(fill="x", padx=5, pady=2)

        ctk.CTkLabel(custom_frame, text="Custom Command:", width=label_width*1.5).grid(row=0, column=0, padx=2, pady=2)
        self.custom_cmd_entry = ctk.CTkEntry(custom_frame, width=entry_width*3)
        self.custom_cmd_entry.grid(row=0, column=1, padx=2, pady=2)

        self.custom_cmd_button = ctk.CTkButton(custom_frame, text="Send", width=button_width)
        self.custom_cmd_button.grid(row=0, column=2, padx=2, pady=2)
    ...
    def setup_output_frame(self):
        ...
        self.output_text = ctk.CTkTextbox(
            ...
            height=400,
            wrap="none",
            font=("Fira Code", 12),  # Primary font
...
def main():
    app = ctk.CTk()  # Initialize the customtkinter app
    app.title("Evil-BW16 Control Panel")
    gui = EvilBW16GUI(app)  # Create an instance of GUI class
